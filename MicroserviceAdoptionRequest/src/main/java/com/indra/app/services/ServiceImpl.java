...
public class ServiceImpl implements IService {
	...
	public boolean insert(long userId, long petId) {
		try {
			...
			if(pet.getAdoptionStatus().equals(AdoptionStatus.AVAILABLE)) {
				...
				throw new NoSuchElementException("Pet not available");
			}
		} catch(NotFound ex) {
			LOGGER.warn("FEIGN NOT FOUND ERROR {}", ex.getMessage());
			throw new NoSuchElementException("Pet or adopter doesn't exists");
		} catch(InternalServerError ex) {
			LOGGER.warn("FEIGN SERVER ERROR {}", ex.getMessage());
	...
	public List<AdoptionRequest> findByEmail(String email) {
		return dao.findByEmailAdopter(email)
				.filter(t->!t.isEmpty())
				.orElseThrow(()->new NoSuchElementException("There's no adopters with that email."));
	}
	...
	public boolean deleteById(long id) {
		...
		throw new NoSuchElementException("There's no adoption with that id.");
	}

	@Override
	public boolean updateAdoptionRequestStatus(long id, AdoptionRequestStatus status) {
		AdoptionRequest request=dao.findById(id).orElseThrow();
